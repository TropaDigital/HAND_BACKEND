...
import {
  makeFakeRole,
  makeFakeRoleList,
  makePrismaRoleRepositoryStub,
} from './helpers/test-helper';

const makeSut = () => {
  const prismaRepository = makePrismaRoleRepositoryStub();
  const sut = new RoleRepository(prismaRepository);

  return { sut, prismaRepository };
};

describe(RoleRepository.name, () => {
  describe(`When ${RoleRepository.prototype.findAll.name} is called`, () => {
    ...
    it('should return prisma result', async () => {
      ...
      expect(result).toEqual(makeFakeRoleList());
    });
  ...
  describe(`When ${RoleRepository.prototype.findById.name} is called`, () => {
    const fakeId = 777;
  ...
  describe(`When ${RoleRepository.prototype.create.name} is called`, () => {
    ...
    it('should call prisma with right params', async () => {
      ...
      expect(createSpy).toBeCalledWith({ data: makeFakeRole({}) });
    });
  ...
  describe(`When ${RoleRepository.prototype.updateById.name} is called`, () => {
    const fakeRole = makeFakeRole({});
    const fakeId = 777;
  ...
  describe(`When ${RoleRepository.prototype.deleteById.name} is called`, () => {
    const fakeId = 777;
